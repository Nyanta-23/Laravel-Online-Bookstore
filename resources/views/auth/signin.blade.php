        ...
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto lg:py-5">
            <div
                ...
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" action="/auth/signin" method="POST">
                        @csrf
                        <div>
                            <label for="email"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white {{ session('authError') || $errors->has('email') ? 'text-red-500' : false }}">Your
                                email</label>
                            <input type="email" name="email" id="email"
                                class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500
                                
                                {{ session('authError') || $errors->has('email') ? '                                border-red-600 focus:outline-none focus:ring-0 focus:border-red-600 text-red-600 placeholder:text-red-600' : false }}
                                "
                        ...
                        <div>
                            ...
                            <input type="password" name="password" id="password" placeholder="••••••••"
                                class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500
                                {{ session('authError') || $errors->has('password') ? 'border-red-600 focus:outline-none focus:ring-0 focus:border-red-600 text-red-600 placeholder:text-red-600' : false }}
                                "
                                required>
